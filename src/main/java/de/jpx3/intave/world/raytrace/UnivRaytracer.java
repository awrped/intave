...
public class UnivRaytracer implements Raytracer {
  @Override
  ...
  public MovingObjectPosition performRaytrace(Player player, Position eyePosition, Position targetPosition) {
    BlockShape eyeShape = shapeAt(player, eyePosition);
    if (!eyeShape.isEmpty()) {
      BlockRaytrace raytrace = eyeShape.raytrace(eyePosition, targetPosition);
      if (raytrace != null) {
        return MovingObjectPosition.fromBlockRaytrace(raytrace, eyePosition, targetPosition);
      }
    }
    ...
    while (hops-- >= 0) {
      ...
      if (!shape.isEmpty()) {
        ...
        if (raytrace != null) {
          return MovingObjectPosition.fromBlockRaytrace(raytrace, contextPosition, targetPosition);
        }
      }
    }
  ...
  private BlockRaytrace innerRaytrace(BlockShape shape, Position eyePosition, Position targetPosition) {
    BlockRaytrace res = shape.raytrace(eyePosition, targetPosition);
    System.out.println("uni | inner block raytrace at " + shape + " -> " + res);
    return res;
  }
